...
## 8.14 Radar Surveillance Layout V1：「80m固定高度航路方向性雷达几何初步划设方案」（本轮）

**目标**：对**已发布运行航路**、在**固定 ALT-080（80 m，EGM2008 正高）**高度层上，用航路周围**现有真实铁塔**作为唯一候选站址，自动选择「雷达型号 + 铁塔 + 单面阵数量 + 单面阵水平方位角」，使海上航路每点至少由 **1** 个不同铁塔站址覆盖、陆地航路每点至少由 **2** 个不同铁塔站址覆盖，且**总单面阵数最少**。**不修改航路本身。**
...
- **高度语义**：航路高度使用既有 ALT-080 / Route3DProfile 的 `egm2008_orthometric` 语义；雷达原点优先真实已知站址/安装高度，否则 `radar_origin_egm2008 = terrain_elevation_egm2008 + radar_mount_height_m`。terrain 复用既有 verified FABDEM DTM 采样（`FabdemWindowTerrainSource`）；**后端绝不写死虚假塔高/安装高度**：缺 mount height 时允许前端提供**统一工程示例参数**，但强制保存 `source` / `confirmed=false` / `parameter_origin=engineering_assumption`；缺关键垂向证据 ⇒ `missing_data`/`pending_confirmation`，**绝不填 0**。
- **俯仰约定（重要）**：`vertical_delta_m = origin_egm2008_m − sample_egm2008_m`，`elevation_deg = atan2(vertical_delta_m, horizontal_distance_m)`。因此要求 `0 <= elevation <= 45` 等价于**雷达原点必须高于被覆盖点**；原点低于目标点会给出负仰角、几何上不可覆盖。这是本模型的真实约束，不是实现细节。
...
- **地表分类**：每个 sample 得到 `surface_class = land | sea | unknown` 与 `required_distinct_site_count`（land=2、sea=1、unknown ⇒ `None`，**fail-closed，绝不自动按 sea 处理**）。陆地判定只用**显式配置的陆域 Polygon**（点在内部**或落在边界上** ⇒ land，保守）；**绝不用 DEM NoData 推断海洋**。项目现有数据里没有任何可用陆域 Polygon（QGIS 工程只有适飞空域 KML/SHP 与建筑图层），因此本轮新增独立 `land_mask` 数据角色与 readiness。
...
- **连续覆盖复核**：25 m 只用于初始优化；得到方案后用 `validation_sample_spacing_m = 5` 对**整条航路**重新做独立复核；发现 `actual_distinct_site_count < required` 即把这些点并入优化输入重新求解；`max_refinement_rounds = 3`。用尽轮次后仍存在遗漏 ⇒ 状态为 `refinement_incomplete`，**不得**报告为 `passed`/`optimal_coverage`。25/5/3 均作为显式软件 baseline 参数与 provenance 保存。
- **结果输出**（`radar_surveillance_layout.items[route_id]`）：`status`、`algorithm_id/version`、`model_scope`、`proposal_only`、`solver{name,library,stage,status,optimality_proven,infeasibility_proven,mip_gap,objective,message,greedy_fallback_used}`、`route_id`、`altitude_layer_id`、`input_fingerprint`、`parameters`（25/5/3 + 俯仰·方位 preset + 固定高度层）、device provenance、`candidate_tower_count`/`candidate_panel_count`、`selected_tower_count`/`selected_panel_count`/`radar_i_panel_count`/`radar_ii_panel_count`、`selected_panels[]`（tower/site_id、radar_type、azimuth_deg、coverage summary）、`validation{land,sea,unknown}`（各含 `length_m`/`satisfied_length_m`/`satisfied_fraction`/`minimum_distinct_site_count`/三态 sample 数）、`uncovered_segments`/`under_redundant_segments`/`unknown_segments`、`infeasibility_reasons[]`、`unknown_evidence[]`、`not_evaluated[]`。`not_evaluated` 显式固定为 `radar_equation`/`Pd_distance_curve`/`terrain_LOS`/`diffraction`/`building_blocking`/`clutter`/`multipath`/`interference`。
- **失效（严格单向）**：`route`/`workspace`/`route_algorithm`/`spatial_3d` 变化、`AltitudeLayer`/`RouteOperatingLayer`/procedure 变化、`towers`/`obstacles` 源或派生事实变化、`terrain_dtm`/`buildings`/`building_grid`/`land_mask` 源变化、以及策略变化，只把 `radar_surveillance_layout` 标 `stale`。它**绝不** stale/改写 routes、CNS、`coverage_3d`、走廊与站址提案；只读投影还会按当前上游输入重算 fingerprint 并自行判定 `stale`。`radar_surveillance_policy` 不变时**不**产生任何失效。
- **API**：`GET /api/radar-surveillance-layout/readiness`、`GET /api/radar-surveillance-policy`、`GET /api/radar-surveillance-layout?route_id=`（完整结果，含逐 sample 明细）、`POST /api/radar-surveillance-policy`、`POST /api/radar-surveillance-layout/evaluate`（一次显式动作 = 一次求解；**无自动生成，也无自动 Apply**）。通用 `/api/state`、`/api/workflow` 只带**有界摘要**（含 selected panels 与合并后的 `coverage_profile`，**不含**逐 sample 明细、**绝不下发未选 panel 的 coverage polygon**），避免地图性能问题。
- **UI**：Step05 新增独立"监视雷达初步划设"结果分段任务卡（route/ALT-080、两型雷达真实资料摘要、mount-height 与 land-mask readiness、25/5/3 参数、运行规划、I-only 与 mixed fallback 状态、selected towers/panels、Ⅰ/Ⅱ 数量、land 双站覆盖率、sea 单站覆盖率、solver optimality 状态、unknown/infeasible 原因），不重构其它页面。地图图层 `radarSurveillanceLayer`（**默认关闭**）只画候选铁塔（弱化）、selected towers、selected panel 在 80 m 平面的方向扇区/环带（inner/outer radius + ±45°，像素半径截断）与**按覆盖结果着色**的航路（satisfied / under_redundant / uncovered / unknown）。
- **求解器不可用**：当前 Python 环境无 `scipy` 时**不得退化为 greedy**，运行时返回 `solver_unavailable` 并给出安装指引；`pyproject.toml` 新增 optional extra `optimization = ["scipy>=1.11,<2"]`（不自动改用户全局环境）。
- **测试**：新增 `tests/test_radar_surveillance_layout.py`（55 项）与 `tests/radar_surveillance_layout_frontend.test.mjs`（10 项），覆盖用户列出的全部 25 项要求（两型 Rmin/Rmax 边界、elevation 0/45 边界与越界、azimuth ±45 边界、359°/0° 环形角、同塔多面阵只算 1 站址、land 双站址/sea 单站址、每塔 ≤4、I-only 可行不启用 II、I-only 被证明不可行才进 mixed、time limit 不算不可行、mixed 先最小总面阵再最少 II、unknown fail-closed、候选方向生成确定性、dominated 剪枝不改变最优覆盖、稀疏优化 + 密集复核发现遗漏、补点重解、轮次用尽不可报完整覆盖、保存/恢复、五类输入变化 stale、不修改 P7/P14-P16 既有结果），并额外覆盖 `solver_unavailable` 语义。
