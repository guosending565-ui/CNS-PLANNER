...
> **BUG-MAP-ROUTE-END-001 取证结论（本轮，未改 planner）**：用户报告"节点/航路端点附近多出一小截深色航路线、端点 marker 落在一条连续线中间"。取证（`tests/test_bug_map_route_end_001.py`，9 项）确认：**`candidate.path[0]` / `candidate.path[-1]` 精确等于 scenario `start` / `end`（端点距离误差恒为 0 m）**；多余的一截来自 **`candidate`（Theta\* V2 权威几何）的 cell-centre connector** —— 设计即 `exact start → source cell centre → Theta* chain → target cell centre → exact end`，当精确端点**不在**格心上时会出现"先回到格心再出发"的可见跳（算例约 470 m，与 L8 格尺度一致）。它不是 operational/scenario 图层叠加，也不是端点不精确。因此：**前端不得做显示期裁剪**（那会让地图几何与 objective / risk / LOS 账本不一致，`layered_candidate_overlay.js` 内已有 `slice(` / `pop()` / `clip` 反向断言锁定）；**未改动 Theta\* V2 语义**。若后续要消除该可见跳，必须按第 13 节规则做成显式版本化变更：`algorithm_version`/`geometry_version` 升级、fingerprint 更新、旧 candidate stale、完整回归，并需人工确认真实 N004 场景。
...
>
> **Theta* V2 正式基线 + Route Safety Evidence V2 决策（最新一轮）**：项目默认 `layered_route_planner` 已由 `layered_route_planner_v1` 提升为 `layered_risk_aware_theta_star_v2@2.0`（**production layered planning baseline**）。新建项目默认 Theta* V2；缺失该 selection 的 legacy 项目在 normalization 时 backfill 到 Theta* V2；**显式保存过 V1 selection 的项目继续运行 V1，绝不静默迁移**。`layered_route_planner_v1` 保留注册、测试、前端与显式选择能力，定位为 **legacy/baseline layered planner**；传统 `route_planner` 默认仍为 `route_planner_v1`（本轮未改）。`heading_bin_count=8` / `theta_min_deg=5.0` 不再是 planner 内部不可见常数：它们随默认 selection 显式持久化在 `algorithm_selection.layered_route_planner.parameters`，并携带 `source=cns_planner_software_algorithm_baseline` / `parameter_origin=software_baseline` / `engineering_confirmed=false` / `evidence=null` / `purpose=search_discretization_and_planning_turn_smoothness_proxy` 的 provenance；candidate / fingerprint / readiness 都能读到 effective parameters + provenance。显式改写后 `parameter_origin=explicit_algorithm_selection`，除非同时提供明确 evidence 与 confirmed，否则绝不自动升级为 engineering confirmed。**`theta_min_deg` 不是航空器最小转弯角，`D_ref` 也不是航空器转弯半径**；Theta* 数学未改变。同轮新增 additive `route_safety_evidence_v2`（post-planning evidence aggregation：不进入 Theta* cost、不改上游、不产生单一 Safety Score）：沿 current published layered operational adoption lineage 聚合 `geometry_obstacle` / `ground_exposure` / `regulatory` / `cns_operational_support` 四个 evidence domain，overall 只表示证据完整性/明确硬约束结果（`evidence_complete` / `evidence_incomplete` / `evidence_unresolved` / `hard_constraint_failed` / `not_ready` / `stale`）。**`confirmed CNS gap = operational support deficit`，不代表 route unsafe，也不改变 geometry validation**；`unknown != safe`、`missing_data != zero`、`validated route != safe route`、`regulatory not configured != passed`。

> **Phase 3.5 工程稳定化决策（本轮，不新增算法）**：三件事，全部在不改变路径规划语义的前提下完成。**(1) 规划终态语义**：`domain/layered_route.py` 现在显式声明 `PLANNING_TERMINAL_STATUSES = (success, no_path, search_incomplete, invalid_input)`——`no_path` 表示**搜索完整跑完**但无可行路径，`search_incomplete` 表示**搜索预算耗尽**（可达性与最优性均未证明，**绝不是空域不可行**），`invalid_input` 表示输入不可解释；`blocked` 保留为历史拼写（仅"显式硬约束阻断端点"），`not_ready`/`missing_data`/`stale` 仍是前置条件。每条非路径结果带 `terminal_status` / `terminal_status_semantics` 与 `blocking_reasons[].{reason_code,resource_limit,reachability_proven,optimality_proven}`。**搜索、heuristic、objective、verdict 均未改动**。**(2) 建筑几何质量**：真实舟山案例 `building=unresolved` 的根因是**提取链路缺陷**——QGIS 投影后单部件 polygon 变成 MultiPolygon（走廊实测 1481/1481），既有 `_metric_ring_of` 只调 `asPolygon()` 而静默返回空环。修复为 `_metric_rings_of()` 逐部件提取 + 新增 `domain/building_geometry_quality.py`（`assess_footprint_geometry` / `prepare_footprint_polygons`：Ring → Polygon → 必要时 `shapely.make_valid` → **全部**有效部件）；`validate_buildings` 与 `building_domain_result` 改为**逐部件**净空判定（多部件不漏判），并在 `evidence.building_quality_report` 输出质量报告（旧键 `geometry_quality` 同时保留）。**源 gpkg 永不改写**（`source_modified=false`），**修复失败保持 unknown**（`building_footprint_quality_unresolved`），`building_clearance` roof 公式与 `evaluate_vertical_clearance` 判据未改。只读报告工具 `tools/building_geometry_quality_report.py`。**(3) 层级能力声明（只声明不改行为）**：`BuildingGridService.capabilities()`（`available_levels=[8]`、`preferred_level=8`、`declared_level_usable`、`unusable_reason`）、`WorkspaceGridService.capabilities()`（1..16、coarsen 语义）、两个 layered planner 的 `PLANNER_CAPABILITY`，以及 `layered_route_planner_readiness().capabilities{planner,building_grid,workspace_grid}`。`level != 8 ⇒ unsupported` 与 coarsen 行为**完全不变**；**层级无关建筑事实获取仍是未完成需求**。详见 `docs/11-Phase3.5稳定化开发报告.md`。

> **Phase 3.5 人工验收修复决策（本轮，只修数据链路与前端显示）**：**未改动** Layered Risk-Aware Theta* V2 搜索/启发/objective/终止判定、`RouteRiskProfile` 数学、`LayeredRouteValidation` 与 `LayeredOperationalAdoption` 的判据与流程。**(1) 建筑事实的层级无关获取（原 Phase 3 §1 未完成项，本轮实现）**：`gis/building_footprint_aggregation.py` 按**当前工作区层级**对**原始 footprint** 做**精确几何聚合**——建筑质心分配计数与高度统计、`EPSG:32651` 米制 `footprint ∩ cell` 精确相交面积，**逐条对齐既有 L8 事实表生成脚本**，`cross_level_averaging=false` / `cross_level_interpolation=false`；产出与 L8 直接映射**键集合完全相同**的 cell，因此 `LayeredFeasibilityAdapter` → Theta* V2 coarse 战略垂向包线**零改动**即可消费。`BuildingGridService.map(grid, source_path)` 的两个位置参数语义不变，L8 路径行为完全不变；新增只读参数 `footprint_source` / `footprint_aggregator` / `footprint_layer_name` 与 additive 载体 `environment_mapping{status∈passed|partial|unsupported,total_cells,covered_cells,unresolved_cells,...}`。无 footprint 源 / 无几何库 / 无空间索引时保持既有 `unsupported` + 逐格 `missing_data`（`unknown != 0`）。**(2) 人口映射统一输出**：`population_mapping` + `coverage_ratio` / `full_cells` / `partial_cells` / `missing_cells`（additive），计数以 `cells` 为唯一权威来源重算；前端不再只显示 `value_status`，改为显示覆盖统计。**(3) 统一路径解析**：`gis/path_resolver.py`（单一格式表，支持 `.qgz`/`.qgs`/`.gpkg`/`.shp`/`.geojson`）、`gis/qgis_project_layers.py`（纯标准库解析 qgz/qgs → 图层 + 绝对路径，远程 datasource 绝不当本地文件）、启动与每次 `metadata()` 的 `exists`/`is_file` 检查（`path_checks`），并修复 `source_manifest` 复用旧 schema/extent 的"缓存旧状态"缺陷。**(4) 建筑轮廓图层（原 FRONT-002）**：`GET /api/building-footprints`（只读，qgz 由后端解析，按视图 bbox + RTree + 简化返回 GeoJSON）+ 前端"建筑轮廓"图层**默认关闭**、LOD 三档（`res>30` 不请求 / `8<res≤30` 上限 600 / `res≤8` 上限 3000），纯显示层、不写 ProjectState、不参与净空判定。详见 `docs/12-Phase3.5验收修复报告.md`。

> **Phase 3.5 二轮人工验收修复决策（AltitudeLayer 目录生命周期）**：Step02 环境建模全部 PASS 后进入 Step03 仍然出现 `altitude layer catalog = not_configured · 共 0 层` ⇒ `resolve_cruise_altitude = altitude_layer_missing` ⇒ Theta* V2 无法执行。根因是**配置生命周期缺陷**：`spatial_3d.altitude_layers` 只有用户手工逐条写入一条产生路径，工作区首次设置与项目恢复都不会补建目录。修复方式：新增 `domain/altitude_layer_defaults.py` 定义**工程默认高度层** `ALT-060 / ALT-080 / ALT-100 / ALT-150 / ALT-200`（60/80/100/150/200 m，`vertical_reference=egm2008_orthometric`，显式 `source`，`confirmed=true`，带边界 40–70 / 70–90 / 90–125 / 125–175 / 175–250 m，各层互不重叠），并在两处幂等接入：`WorkspaceService.set_workspace`（工作区确认时补建并提交）与 `WorkflowSession._load`（项目恢复时补建、**不写盘**，只置 `pending_save`，因此打开项目仍匹配既有只读持久化契约）。判定条件 = 已有 workspace + grid cells + 目录为空 + `altitude_layer_defaults_initialized` 未置位；首次补建或被**显式删除**时置位，用户刻意删空的目录在后续恢复中**保持为空**并由前端显式提示，绝不静默填满。**边界（绝不放宽高度检查）**：补建的只是 **catalog 条目**，planning request 的 `altitude_layer_id` 仍必须由用户显式选择（`parameter_status=no_default_altitude_layer`、`altitude_layer_not_explicitly_selected` 语义不变），不触碰 route 分配 / procedure / 规划请求 / feasibility·cost policy / candidate / validation / adoption；Theta* V2、`RouteRiskProfile`、`LayeredRouteValidation`、`LayeredOperationalAdoption` 一律未改。前端（`layered_theta_v2.js` ① 区块 + `layered_route_planner.js` 下拉）在目录为空时给出明确提示，但**不放行**门控（运行按钮仍只由后端 `theta_star_v2.blockers` 决定）。回归：`tests/test_altitude_layer_lifecycle.py`（14 项）+ 前端 2 项。详见 `docs/12-Phase3.5验收修复报告.md` §4b。
...
- `gis/source_loader.py`、`renderer.py`、`constraints.py`、`raster_adapter.py`、`airspace_adapter.py`：QGIS/GDAL 边界。`OPTIONAL_VECTOR_SOURCE_KEYS` 含 `land_mask`（陆域掩膜只需是存在的受支持矢量文件；逐点判定在 radar adapter 内完成，**不**要求解析出建筑图层）。
- **数据源角色（V1.1 新增 `land_mask`）**：`data.registry` 定义 `land_mask`（三维建筑环境 / vector / GPKG·SHP·GeoJSON / "Radar Surveillance Layout V1.1 的 land|sea 判定来源；未配置时 surface_class 保持 unknown（fail-closed）"）；`MapData.DEFAULT_PATHS['land_mask'] = D:/aaa2026project/UOM/舟山/规划系统/GLO30/boundary/zhejiang_boundary.gpkg` 只是**本机默认建议值**（`layer_name = zhejiang_boundary`、`source_crs = EPSG:4326`、发现自 QGIS 工程 `.../GLO30/11.qgz`；工程化/简化省域边界，**不是** 5 m 高精度海岸线）。`ProjectDirectoryService.SPATIAL_SOURCE_KEYS` 与 `/api/data-sources` clean 白名单都包含 `land_mask`，Save As/Open Project 必须原样带回。
...
- `web/js/state/workflow_snapshot.js`：**唯一**允许写入全局 `flow` 的路径（BUG-GRID-POPUP-001）。通用 `/api/workflow` 快照为性能做 `slim_grid_attributes()`（**不含** `grid_attributes.*.cells`），因此 `applyWorkflow` / `mutate` / `refreshWorkflow` / `resourceAction` / 打开项目 / bootstrap 都必须经过 `applyWorkflowSnapshot`：先安装 snapshot，再按 `detail_available` 从 `GET /api/workspace/grid` + `GET /api/workspace/grid/attributes` **hydrate 逐 cell 明细**，最后才 render/paint。竞态由单调递增的 `gridDataSerial` 独占裁决（旧响应一律丢弃，绝不覆盖新工作区）；`populationCellStructureDiagnostics` 在 mapping `status=passed` 但某个 `grid_id` 缺记录时显式报告 `population_cell_missing_for_grid_id`，绝不静默退化为"缺少数据"。**通用快照继续保持 slim**，不为了 popup 把上万 cells 塞回 `/api/workflow`。
...
## 8.14 Radar Surveillance Layout V1.1：「80m固定高度航路方向性雷达几何初步划设方案」

**目标**：对**已发布运行航路**、在**固定 ALT-080（80 m，EGM2008 正高）**高度层上，用航路周围**现有真实铁塔**作为唯一候选站址，自动选择「雷达型号 + 铁塔 + 单面阵数量 + 单面阵水平方位角」，使海上航路每点至少由 **1** 个不同铁塔站址覆盖、陆地（含海岸不确定带）航路每点至少由 **2** 个不同铁塔站址覆盖，且**总单面阵数最少**。**不修改航路本身。**

> **V1.0 → V1.1（本轮，三个 P0 几何语义修复 + 陆域真实接入）**
>
> 1. **BUG-RADAR-ALT-001（航路采样高度）**：航路是 80 m 固定巡航高度航路，因此**所有** optimization / validation / refinement 采样点的 `sample.egm2008_m` **恒等于 80.0**。V1.0 把 FABDEM 地面正高当作航路高度代入是**错误**的，该输入通道已从 `build_route_samples` 签名中**删除**。FABDEM 仍用于既有 `tower_obstacle_profiles`（塔底/塔顶派生）与未来 terrain LOS，但**不再**决定 route sample z。测试证明 terrain A = 10 m 与 terrain B = 60 m 给出**完全相同**的 sample 高度与覆盖结论。
> 2. **BUG-RADAR-ELEV-002（俯仰符号）**：唯一正确语义改为 `vertical_delta_m = sample_egm2008_m − radar_origin_egm2008_m`（**目标减雷达**，不再是 `origin − sample`）。因此目标高于雷达 ⇒ 正仰角；等高 ⇒ 0°；低于雷达 ⇒ 负仰角 ⇒ 本模型（不下倾）**不覆盖**。真实示例：route = 80 m、radar = 50 m、horizontal = 1000 m ⇒ elevation ≈ **+1.718°**（可覆盖）；radar = 110 m ⇒ vertical_delta = −30 m ⇒ **不覆盖**；45° 边界 ⇔ `sample − origin == horizontal`。`geometry.py`、candidates、domain provenance、application result、前端文案与原有锁定错误语义的单测**同步改为正确语义**（绝不为了原测试 passed 而保留错误物理模型）。
> 3. **BUG-RADAR-ORIGIN-003（雷达原点）**：`radar_origin_egm2008_m = tower_top_orthometric_m`（`tower_top_orthometric_m` 已代表真实塔顶正高：ground tower = FABDEM + 塔身；rooftop tower = FABDEM + 建筑高度 + 塔身）。V1.0 又在塔顶之上叠加 `radar_mount_height_m = 30 m`（= "塔顶以上 30 m"）是**错误**的，该叠加已删除。记录 `radar_origin_basis = tower_top_orthometric_m` / `installation_assumption = radar_phase_center_at_tower_top` / `engineering_confirmed = false`。legacy `radar_mount_height_m` **允许兼容读取**，但恒被标记 `legacy_not_used_by_v1_1`（`used_by_algorithm_version = null`、`required_for_v1_1 = false`）且**不影响** V1.1 几何；**不再作为 readiness 必填项**。`tower_top_orthometric_m` 未解析的塔继续**不可作为候选**，**绝不填 0**。
> 4. **陆域真实接入（BUG-LANDMASK-SOURCE-001 / PERSIST-001 / CRS-002）**：真实来源为 `D:\aaa2026project\UOM\舟山\规划系统\GLO30\boundary\zhejiang_boundary.gpkg`（`layer_name = zhejiang_boundary`、`source_crs = EPSG:4326`、`Polygon/MultiPolygon`，发现自 QGIS 工程 `...\GLO30\11.qgz`）。该路径只作为本机 `MapData.DEFAULT_PATHS["land_mask"]` 建议值；**算法与 GIS 适配器一律从 `data_sources["land_mask"]` 消费，绝不硬编码绝对路径**。`ProjectDirectoryService.SPATIAL_SOURCE_KEYS` 与 `/api/data-sources` 的 clean 白名单**都已加入 `land_mask`**，因此 Save As → 关闭 → 重开不会丢失该路径。`LandMaskSource` **不再假定所有源都是 4326**：GPKG 从 `gpkg_geometry_columns.srs_id` 读真实 CRS、其它格式走 GDAL/pyproj；几何与查询点都经**显式**变换，并且这对变换要经过**往返自校验**（非米级量级 / 往返失败 / 源 CRS 不可解析 / 几何与声明的投影 CRS 不符 ⇒ `unknown`，**fail-closed**），**禁止 degree-as-meter**。
> 5. **海岸不确定带（第 6 节）**：`coastal_uncertainty_buffer_m = 30.0`，`parameter_origin = engineering_assumption`、`confirmed = false`，可配置并进入 provenance/fingerprint。分类规则（全部在**明确米制 CRS** 中完成）：polygon 内或边界上 ⇒ `land`（要求 2 站址）；外侧但距边界 ≤ buffer ⇒ `coastal_uncertain`（`effective_requirement_class = land`，要求 2 站址，`classification_confidence = uncertain`）；外侧且距离 > buffer ⇒ `sea`（要求 1 站址）；CRS/几何/源不可用 ⇒ `unknown`（要求数 `None`，fail-closed）。**该 30 m 绝不能被描述成数据真实精度**，也不等同于 5 m validation resolution。
> 6. **BUG-RADAR-REFINE-003（25 m / 5 m 分类独立）**：5 m validation sample **重新生成真实 5 m 位置**并**再次独立**执行 land/sea/coastal 分类；refinement point 保留自己的精确 classification。**禁止** nearest 25 m surface_class inheritance（`route_sampling.nearest_sample_classification_inheritance = false`）。测试构造"海岸线夹在两个 25 m 点之间"的算例证明 5 m 层能独立检测 `sea → coastal_uncertain → land` 变化。
> 7. **BUG-RADAR-READINESS-004**：`passed` 至少要求——≥1 条 passed operational route；ALT-080 存在且 confirmed/current；真实铁塔存在；`tower_obstacle_profiles` current；≥1 个已解析 `tower_top_orthometric_m`；land_mask 已配置且文件/图层/Polygon 可用、源 CRS 可解析、分类转换可用；`EPSG:32651` 米制变换可用；scipy/HiGHS solver 可用（evaluate 前即可明确报告）。**不再要求 mount height configured**。land_mask 缺失/CRS 失败 ⇒ readiness 必须 `not_ready`（不会先显示 passed、运行时才全 unknown）。
> 8. **BUG-RADAR-OVERLAY-005（80 m 平面扇区绘制）**：Radar-I 的 120/3000 m 与 Radar-II 的 200/5000 m 是**斜距**，前端**绝不**再当作 80 m 平面的水平半径。后端为每个 selected panel 输出 `dz_m = 80.0 − radar_origin_egm2008_m`、`horizontal_outer_radius_m = sqrt(max(0, Rmax² − dz²))`、`horizontal_inner_radius_m = max(dz, sqrt(max(0, Rmin² − dz²)))`；`dz < 0` 或无有效交截 ⇒ `plane_intersection_status = "no_intersection"` 且半径为 `null`（前端不画该扇区，并在面板上**显式说明原因**，绝不静默少画）。
> 9. **algorithm_version 1.0 → 1.1**：本轮改变真实几何语义，因此版本升级；`input_fingerprint` 至少加入 `route_altitude_semantics = fixed_alt_080_egm2008`、`vertical_delta_semantics = target_minus_radar_origin`、`radar_origin_semantics = tower_top_orthometric`、`land_mask_semantics/version`、`coastal_uncertainty_buffer_m`、`geometry_version = radar_layout_geometry_v1_1`。旧 V1.0 layout 保留为历史记录但必然 `stale`，且**只** stale `radar_surveillance_layout`，**不向 upstream route/CNS 传播**。
>
> **本轮未改**：Radar-I 120~3000 / Radar-II 200~5000 斜距、方位 ±45°、俯仰 0~45°（center 22.5°）、每塔最多 4 个单面阵、land ≥2 / sea ≥1 distinct tower site、同塔多面阵对同一点只贡献 1 个 site、Stage A（I-only, min total panels）→ 仅 solver 明确 proven infeasible 才进 Stage B（time/node limit **绝不**当 infeasible）、B1 min total panels / B2 固定 N* 再 min Radar-II、`scipy.optimize.milp + HiGHS`（**禁止** greedy fallback）、25 m → 5 m → 违例点追加 → ≤3 轮 refinement。

- **独立 additive**，`model_scope = geometric_initial_radar_layout`，`proposal_only = true`。新增 `domain/radar_surveillance_layout.py`、`algorithms/radar_layout/{geometry,candidates,milp,v1}.py`、`gis/radar_layout_adapter.py`、`application/radar_surveillance_layout_service.py`；ProjectState 独立保存 `radar_surveillance_policy` / `radar_surveillance_layout`。**绝不自动写** `existing_cns_facilities` / `coverage_3d` / `cns_service_capability` / `cns_corridor_assessment` / `cns_corridor_gap_assessment` / `cns_corridor_site_plan`，也不改设备目录里的其它 C/N/S 数据；`GeometricCoverage3DV1` 的 sphere/hemisphere 语义、Theta\* V2、`RouteRiskProfile`、`LayeredRouteValidation`/Adoption、`Route3DProfile` 一律未改。
- **真实设备资料（只读来源）**：`D:\aaa2026project\UOM\舟山\基础数据\设备性能\低空智能网联系统相关设备信息-四创(2).docx`（**是文件**，不是目录；SHA-256 `e0d9cc20f19b74d30df34548274fb98f04f36bb61b54531bda5db638f13f0b81`）。只提取**中近程雷达Ⅰ型 / Ⅱ型**两个表格条目；`source_modified=false`，同级目录其它设备资料（四创(1)、远程雷达、超近程雷达、气象设备等）显式记录为 `same_directory_not_used_for_substitution`。资料原文与单位逐字段保存（`raw`/`value`/`unit`/`quantity`/`source_row`/`condition`），`used_in_geometry=false`。
- **固定几何参数（本轮模型值）**：Radar-I `min_slant_range_m=120` / `max_slant_range_m=3000`；Radar-II `200` / `5000`；共同 `azimuth_beamwidth_deg=90`、`azimuth_half_width_deg=45`、`elevation_center_deg=22.5`、`elevation_min_deg=0`、`elevation_max_deg=45`。资料中的探测距离是**不等式**（Ⅰ型 `≤120米`/`≥3km`，Ⅱ型 `≤200米`/`≥5km`），`geometry_parameter_mapping` 同时保存 `source_inequality` 与 `source_value`，因此模型值与来源证据可逐项对照。`RCS_reference_m2=0.01`、`Pd_reference=0.8`、`Pfa_reference=1e-6` 以及方位/俯仰/距离精度、搜索/跟踪更新时间、最大探测高度、分类识别率**只进设备事实与 provenance，不进入几何布设公式**。
- **高度语义（V1.1）**：航路是 **80 m 固定巡航高度航路**，`sample.egm2008_m ≡ 80.0`（`vertical_reference = egm2008_orthometric`，`route_sample_height_semantics = fixed_alt_080_egm2008_constant_for_every_sample`，`terrain_elevation_used_as_route_height = false`）。**地形高程不进入航路采样高度**。雷达原点 `radar_origin_egm2008_m = tower_top_orthometric_m`（塔顶即相位中心，`engineering_confirmed = false`），**不再**叠加任何挂高；legacy `radar_mount_height` 只作兼容读取（`legacy_not_used_by_v1_1`）。**后端绝不写死虚假塔高/安装高度**；缺关键垂向证据 ⇒ `missing_data`/`not_ready`，**绝不填 0**。
- **俯仰约定（V1.1，重要）**：`vertical_delta_m = sample_egm2008_m − origin_egm2008_m`（**目标减雷达**），`elevation_deg = atan2(vertical_delta_m, horizontal_distance_m)`。因此要求 `0 <= elevation <= 45` 意味着**目标必须不低于雷达原点**（本模型不下倾）：目标高于雷达 ⇒ 正仰角可覆盖；目标低于雷达 ⇒ 负仰角不可覆盖；等高 ⇒ 0°。这是本模型的真实约束，不是实现细节。
- **航路采样**：**不采用**"每个 MH/T 栅格只取一个航路点"。直接沿实际 operational route 里程采样，`optimization_sample_spacing_m = 25`；距离/角度全部在**显式米制空间**中完成（复用项目既有 `EPSG:32651` 机制，`QgisMetricTransform`），算法包只消费米制坐标，**禁止 degree-as-meter**；MH/T Grid **不**作为雷达覆盖离散基础。
- **地表分类（V1.1）**：每个真实采样点在自己的位置上独立得到 `surface_class = land | coastal_uncertain | sea | unknown` 与 `required_distinct_site_count`（land=2、coastal_uncertain=2（按 land）、sea=1、unknown ⇒ `None`，**fail-closed，绝不自动按 sea 处理**）。陆地判定只用**显式配置的陆域 Polygon**（点在内部或落在边界上 ⇒ land，保守；多边形**内洞保留**，不挖洞）；海岸带按显式 `coastal_uncertainty_buffer_m` 单独成类；**绝不用 DEM NoData 推断海洋**。25 m 与 5 m 两条采样链**各自**在自己的真实位置分类，**禁止最近邻继承**。
- **候选单面阵方向（确定性，不逐度穷举）**：对每个 `tower × radar_type`，(1) 先按 range/elevation 筛物理可达 sample；(2) 计算 bearing；(3) 候选中心方向 = `bearing`、`bearing−45°`、`bearing+45°`；(4) normalize 到 `[0,360)`；(5) 数值容差去重；(6) 计算每个方向的 coverage sample set；(7) **相同 coverage set 只留一个确定性代表**；(8) 删除同塔同型号中 coverage set 被另一候选**完全包含**的 dominated panel。0°/360° 环绕按最短环形夹角处理。
- **MILP（`scipy.optimize.milp` + HiGHS，scipy.sparse 结构）**：变量 `x_p ∈ {0,1}`（是否安装 panel）与 `y_tj ∈ {0,1}`（不同 tower/site 是否通过至少一个已选 panel 覆盖 sample j）；约束 `sum_{p∈t} x_p <= 4`（每塔最多 4 个单面阵）、`y_tj <= sum(A_pj · x_p)`、`sum_t y_tj >= required_j`。该 tower 不存在任何可覆盖 j 的 panel 时不创建无意义变量（默认上界 0）。**不使用 greedy 冒充最优解**；`y` 只作线性化辅助，最终 `actual_distinct_site_count` 一律**按 selected panels 重算**。
- **两阶段词典序**：Stage A 只用 Radar-I，`min total_panel_count`；**只有 solver 明确证明 `infeasible` 才进入 Stage B**（`time_limit`/`iteration_limit`/`node_limit`/`solver_error` 一律 `search_incomplete`/`solver_error`，**不得**解释为 I 型不可行，也不自动进入 II 型）。Stage B：B1 `min total_panel_count` 得 `N*`，B2 增加 `total_panel_count = N*` 等式约束后 `min Radar-II panel count`。两次 solve 的 `status`/`objective`/`mip_gap` 全部记录。
- **求解前不可行检查**：MILP 前逐 sample 计算 `candidate_distinct_site_count`；若小于 `required_distinct_site_count` 直接形成**结构化 infeasibility evidence**（sample id / surface_class / 候选数 / 要求数），I-only 与 mixed 阶段分别记录，绝不只返回"求解失败"。
- **连续覆盖复核**：25 m 只用于初始优化；得到方案后用 `validation_sample_spacing_m = 5` 对**整条航路重新生成真实 5 m 位置**并**再次独立分类**复核；发现 `actual_distinct_site_count < required` 即把这些点并入优化输入重新求解；`max_refinement_rounds = 3`。用尽轮次后仍存在遗漏 ⇒ 状态为 `refinement_incomplete`，**不得**报告为 `passed`/`optimal_coverage`。25/5/3 均作为显式软件 baseline 参数与 provenance 保存。
- **结果输出**（`radar_surveillance_layout.items[route_id]`）：`status`、`algorithm_id/version`（`1.1`）、`model_scope`、`proposal_only`、`semantics_fingerprint`、`solver{name,library,stage,status,optimality_proven,infeasibility_proven,mip_gap,objective,message,greedy_fallback_used}`、`route_id`、`altitude_layer_id`、`input_fingerprint`、`parameters`（25/5/3 + 俯仰·方位 preset + 固定高度层 + V1.1 语义）、device provenance、`radar_origin{radar_origin_basis,semantics,installation_assumption,engineering_confirmed,legacy_mount_height_used=false,...}`、`sources{terrain_dtm,land_mask,radar_origin,radar_mount_height}`、`land_mask_source_provenance`、`surface_classification{surface_class_counts,validation_surface_class_counts,classification_independence,...}`、`route_sampling{sample_egm2008_m=80,...}`、`candidate_tower_count`/`candidate_panel_count`、`selected_tower_count`/`selected_panel_count`/`radar_i_panel_count`/`radar_ii_panel_count`、`selected_panels[]`（tower/site_id、radar_type、azimuth_deg、`radar_origin_egm2008_m`、`altitude_plane_geometry{dz_m,...}`、`horizontal_inner_radius_m`/`horizontal_outer_radius_m`/`plane_intersection_status`、`slant_range_preset_m`、coverage summary）、`validation{land,sea,coastal_uncertain,unknown}`（各含 `length_m`/`satisfied_length_m`/`satisfied_fraction`/`minimum_distinct_site_count`/三态 sample 数）、`uncovered_segments`/`under_redundant_segments`/`unknown_segments`、`infeasibility_reasons[]`、`unknown_evidence[]`、`not_evaluated[]`。`not_evaluated` 显式固定为 `radar_equation`/`Pd_distance_curve`/`terrain_LOS`/`diffraction`/`building_blocking`/`clutter`/`multipath`/`interference`。
- **失效（严格单向）**：`route`/`workspace`/`route_algorithm`/`spatial_3d` 变化、`AltitudeLayer`/`RouteOperatingLayer`/procedure 变化、`towers`/`obstacles` 源或派生事实变化、`terrain_dtm`/`buildings`/`building_grid`/`land_mask` 源变化、以及策略变化（含 `coastal_uncertainty_buffer_m` / `land_mask_layer_name`），只把 `radar_surveillance_layout` 标 `stale`。它**绝不** stale/改写 routes、CNS、`coverage_3d`、走廊与站址提案；只读投影还会按当前上游输入重算 fingerprint 并自行判定 `stale`。`radar_surveillance_policy` 不变时**不**产生任何失效。**V1.0 layout 必然 stale**（`algorithm_version` + `semantics_fingerprint` 都在输入指纹里），且该 stale 只作用于本产物。
- **API**：`GET /api/radar-surveillance-layout/readiness`、`GET /api/radar-surveillance-policy`、`GET /api/radar-surveillance-layout?route_id=`（完整结果，含逐 sample 明细）、`POST /api/radar-surveillance-policy`、`POST /api/radar-surveillance-layout/evaluate`（一次显式动作 = 一次求解；**无自动生成，也无自动 Apply**）。通用 `/api/state`、`/api/workflow` 只带**有界摘要**（含 selected panels、`coastal_uncertain`、`land_mask_source_provenance`、`surface_classification`、`route_sampling` 与合并后的 `coverage_profile`，**不含**逐 sample 明细、**绝不下发未选 panel 的 coverage polygon**），避免地图性能问题。
- **UI（V1.1）**：Step05 独立"监视雷达初步划设"任务卡显示——航路 / ALT-080 / 80 m EGM2008；land mask（`zhejiang_boundary` / `EPSG:4326` / `explicit_polygon`）；海岸不确定带 30 m + `engineering_assumption` + `confirmed=false`；candidate towers / resolved tower tops；radar origin（`tower_top_orthometric_m` / `radar_phase_center_at_tower_top` / `engineering_confirmed=false`）与 legacy 挂高（只读、标注 `legacy_not_used_by_v1_1`）；I-only status / mixed fallback status；selected tower count / selected panel count / Radar-I count / Radar-II count；land / sea / coastal_uncertain 的 satisfied length+fraction 与 unknown length；minimum distinct-site coverage；solver status / `optimality_proven` / `infeasibility_proven` / `mip_gap` / `message`。地图图层 `radarSurveillanceLayer`（**默认关闭**）只画候选铁塔（弱化）、selected towers 突出、**只画 selected panel**（使用后端 80 m 水平交截半径，绝不使用斜距），航路按 satisfied / under_redundant / uncovered / unknown 着色；无有效平面交截的 panel 不画并在面板上说明原因。不在一次绘制中画任何未选 panel。
- **求解器不可用**：当前 Python 环境无 `scipy` 时**不得退化为 greedy**，readiness 与运行时都明确报告 `solver_unavailable` 并给出安装指引；`pyproject.toml` 有 optional extra `optimization = ["scipy>=1.11,<2"]`（不自动改用户全局环境）。
- **测试（V1.1）**：`tests/test_radar_surveillance_layout.py`（68 项）与 `tests/radar_surveillance_layout_frontend.test.mjs`（11 项），除原有 25 项要求外新增覆盖：`sample.egm2008_m ≡ 80.0`（terrain A/B 结论完全相同）、`build_route_samples` 签名不再有地形高度回调、`vertical_delta = target − origin` 的正/零/负三态与真实示例（+1.718°）、`radar_origin == tower_top_orthometric_m`（legacy 挂高**不改变**原点、塔顶未解析仍不可用）、海岸不确定带 A/B/C/D 四类与显式 0 / 更大 buffer、`coastal_uncertain` 要求 2 站址、真实 GPKG 的**逐图层 CRS 读取**（4326 vs 32651 同名几何分类一致）与 CRS 不可解析/几何与声明不符 ⇒ `unknown`、内洞保留、25 m/5 m 分类独立、80 m 平面交截半径数学 + `no_intersection` 语义、`algorithm_version == 1.1` 与指纹分量、readiness 不再要求挂高但要求 land mask、V1.0→V1.1 stale。
