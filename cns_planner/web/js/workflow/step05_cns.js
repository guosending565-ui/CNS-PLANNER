...
const RESULT_SEGMENTS=[['cns-res-coverage','三维覆盖评估'],['cns-res-capability','服务能力评估'],['cns-res-corridor','CNS 服务走廊'],['cns-res-gap','CNS 能力缺口'],['cns-res-site','CNS 设施规划'],['cns-res-radar','监视雷达初步划设']];
...
export function render({flow}){
  ...
  if(legacyCoverage.status&&legacyCoverage.status!=='not_calculated')result=Object.entries(legacyCoverage.layers||{}).map(([key,layer])=>{const stats=layer.statistics;return '<div class="coverage-card"><b>'+key+' '+statusBadge(layer.status)+'</b><span>站点 '+stats.stations+' · 主站 '+stats.primary+' · 补盲 '+stats.gap+' · 共址 '+stats.colocated+'</span><span>平均重数 '+stats.average_multiplicity+' · 未覆盖 '+stats.uncovered_samples+'</span></div>';}).join('');
  ...
  const coverageResult='<h3>旧版二维覆盖试算（不用于正式规划） '+statusBadge(legacyCoverage.status||'not_calculated')+'</h3><div class="parameter-note">旧版兼容结果仅保存在当前会话，不写入项目状态，不决定正式规划是否完成。</div><button class="secondary full" id="planCoverage">运行旧版二维覆盖试算</button><div class="coverage-results">'+result+'</div>';
  ...
  const coverage3dPanel='<h3>三维几何覆盖评估 '+statusBadge(coverage3d.status||'not_calculated')+'</h3><div class="parameter-note">几何覆盖 ≠ 真实 CNS 性能；传播、视距、绕射、干扰、链路预算和传感器探测概率均未评估。</div><label>采样间距（米）<input class="panel-input" type="number" id="coverage3dSpacing" value="'+(coverage3d.parameters?.sample_spacing_m||flow.algorithm_selection?.coverage_model?.parameters?.sample_spacing_m||500)+'"></label><button class="secondary full" id="evaluateCoverage3d">运行三维几何覆盖</button><div class="gap-results">'+coverage3dList(coverage3d)+'</div>';
  const capabilityPanel='<h3>CNS 服务能力评估 '+statusBadge(capability.status||'not_calculated')+'</h3><div class="parameter-note">静态能力满足不等于当前服务可用；视距、绕射、干扰、负载和切换等尚未评估。</div><button class="secondary full" id="evaluateServiceCapability">评估 CNS 服务能力</button><div class="gap-results">'+capabilityList(capability)+'</div>';
  const corridorPanel='<h3>CNS 服务走廊 '+statusBadge(corridor.status||'not_calculated')+'</h3><div class="parameter-note">这是工程 CNS 服务需求走廊，不是法规批准空间；水平范围采用保守网格纳入，体积为离散体积代理。</div><button class="secondary full" id="evaluateCorridor">评估 CNS 服务走廊</button><div class="gap-results">'+corridorSummary(corridor)+'</div>';
  const objectivesGapPanel=planningObjectivesPanel(flow)+'<div class="parameter-note">Spatial continuous-deficit 是 corridor voxel 的保守纵向投影，不是 runtime outage、正式 ICAO continuity 或 availability probability。</div><div class="gap-results">'+corridorGapSummary(corridorGap)+'</div>';
  // ---- 高级：走廊站址优化 / 运行时间线 / 保护与 Gap V2 / Legacy与闭环 -------
  const corridorSitePlanPanel='<h3>CNS 设施规划 '+statusBadge(corridorSitePlan.status||'not_calculated')+'</h3><div class="parameter-note">仅针对已确认的走廊缺口目标，通过累计试算验证服务与独立冗余收益；证据不足不会触发建站。</div><label class="check-row"><input type="checkbox" id="corridorSitePolicyConfirmed" '+(corridorSitePolicy.confirmed?'checked':'')+'> 确认走廊复用优先规划策略</label><button class="secondary full" id="evaluateCorridorSitePlan">生成 CNS 设施规划方案</button><div class="gap-results">'+corridorSitePlanSummary(corridorSitePlan)+'</div>';
  ...
  const sitePlanPanel='<h3>旧版站址试算（不用于正式规划） '+statusBadge(sitePlan.status||'not_calculated')+'</h3><div class="parameter-note">旧版兼容试算仅保存在当前会话，不写入项目状态，不解锁方案评审。复用层级顺序：Existing CNS → Existing Shared Site → Tower Colocation Host（真实铁塔共塔宿主）→ Candidate Site → New-build Candidate；该顺序表示 prefer 共塔而不是 force。</div><label class="check-row"><input type="checkbox" id="sitePolicyConfirmed" '+(sitePolicy.confirmed?'checked':'')+'> 确认旧版复用优先试算策略</label><button class="secondary full" id="evaluateSitePlan">运行旧版站址试算</button><div class="gap-results">'+sitePlanSummary(sitePlan)+'</div>';
  const closedLoopPanel='<h3>高级：方案影响试算 '+statusBadge(closedLoop.status||'not_calculated')+'</h3><div class="parameter-note">只在工作副本比较方案前后影响；复核操作不会写入正式覆盖、设施、确认方案或运行航路。</div><div class="button-row"><button class="secondary" id="evaluateClosedLoop">生成影响试算</button><button class="secondary" id="applyClosedLoop" '+(closedLoop.validation_status==='validated_improvement'&&closedLoop.commit_status==='preview'?'':'disabled')+'>复核试算结果（不写入正式项目）</button></div><div class="gap-results">'+closedLoopSummary(closedLoop)+'</div>';
  const body=wbPanel('operate','',{segments:[
    ['cns-op-devices','设备与参数',
      wbBlock('设备与参数',wbSegHint(OPERATE_SEGMENTS,'cns-op-devices')+deviceCatalogNote+equipmentReferencePanel(flow)+engineeringParameters)
      +wbBlock('设备参数与布站',deviceActions)],
    ['cns-op-existing','已有设施',wbBlock('已有设施',wbSegHint(OPERATE_SEGMENTS,'cns-op-existing')+existingPanel)],
    ['cns-op-candidates','候选站址',wbBlock('候选站址',wbSegHint(OPERATE_SEGMENTS,'cns-op-candidates')+candidatePanel)]
  ]})
    +wbPanel('result','',{segments:[
      ['cns-res-coverage','三维覆盖评估',wbBlock('三维覆盖评估',wbSegHint(RESULT_SEGMENTS,'cns-res-coverage')+coverage3dPanel)],
      ['cns-res-capability','服务能力评估',wbBlock('服务能力评估',wbSegHint(RESULT_SEGMENTS,'cns-res-capability')+capabilityPanel)],
      ['cns-res-corridor','服务走廊',wbBlock('服务走廊',wbSegHint(RESULT_SEGMENTS,'cns-res-corridor')+corridorPanel)],
      ['cns-res-gap','CNS 能力缺口',wbBlock('CNS 能力缺口',wbSegHint(RESULT_SEGMENTS,'cns-res-gap')+objectivesGapPanel)],
      ['cns-res-site','CNS 设施规划',wbBlock('CNS 设施规划',wbSegHint(RESULT_SEGMENTS,'cns-res-site')+corridorSitePlanPanel)],
    ...
    +wbPanel('advanced','',{segments:[
      ...
      ['cns-adv-gapv2','保护与 Gap V2',wbBlock('保护与 Gap V2',wbSegHint(ADVANCED_SEGMENTS,'cns-adv-gapv2')+protectionPanel+gapV2Panel)],
      ...
      ['cns-adv-closedloop','高级方案影响试算',wbBlock('高级方案影响试算',wbSegHint(ADVANCED_SEGMENTS,'cns-adv-closedloop')+closedLoopPanel)
        +'<div class="flow-summary">生命风险：'+statusText(flow.risks.life.status)+' · 财产风险：'+statusText(flow.risks.property.status)+'<br>高级与旧版兼容结果均不用于正式规划门禁。</div>']
    ]})
    +'<button class="primary full" id="nextStep" '+(!flow.steps['5']?'disabled':'')+'>下一步：方案评审</button>';
  return shell('05','CNS规划','三维覆盖评估 → 服务能力评估 → CNS 服务走廊 → CNS 能力缺口 → CNS 设施规划。',body);
}
