...
export function render({flow}){
  ...
  const coverage3dPanel='<h3>3D Geometric Coverage '+statusBadge(coverage3d.status||'not_calculated')+'</h3><div class="parameter-note">几何覆盖 ≠ 真实 CNS 性能；传播、LOS、绕射、干扰、链路预算和传感器 Pd 均未评估。</div><label>sample spacing (m)<input class="panel-input" type="number" id="coverage3dSpacing" value="'+(coverage3d.parameters?.sample_spacing_m||flow.algorithm_selection?.coverage_model?.parameters?.sample_spacing_m||500)+'"></label><button class="secondary full" id="evaluateCoverage3d">运行 3D 几何覆盖</button><div class="gap-results">'+coverage3dList(coverage3d)+'</div>';
  const capabilityPanel='<h3>CNS Service Capability '+statusBadge(capability.status||'not_calculated')+'</h3><div class="parameter-note">静态能力满足 ≠ 当前服务 available。模型成熟度：engineering baseline；LOS/绕射/干扰/负载/切换等未评估。</div><button class="secondary full" id="evaluateServiceCapability">评估技术感知静态能力</button><div class="gap-results">'+capabilityList(capability)+'</div>';
  const corridorPanel='<h3>CNS Service Requirement Corridor '+statusBadge(corridor.status||'not_calculated')+'</h3><div class="parameter-note">工程 CNS 服务需求走廊，不是 JARUS Operational Volume、U-space Surveillance Volume 或法规批准空间。水平为保守网格纳入，体积为 discretized volume proxy。</div><button class="secondary full" id="evaluateCorridor">评估 Corridor Voxels</button><div class="gap-results">'+corridorSummary(corridor)+'</div>';
  ...
  const body=wbPanel('operate','',{segments:[
    ...
    +wbPanel('advanced','',{segments:[
      ['cns-adv-site','走廊站址优化',wbBlock('走廊站址优化',wbSegHint(ADVANCED_SEGMENTS,'cns-adv-site')+corridorSitePlanPanel)],
      ...
      ['cns-adv-closedloop','Legacy与闭环',wbBlock('Legacy与闭环',wbSegHint(ADVANCED_SEGMENTS,'cns-adv-closedloop')+sitePlanPanel+closedLoopPanel)
