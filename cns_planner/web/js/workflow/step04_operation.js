...
function capabilitySummary(item){
  ...
  const summary=(label,data)=>label+' type['+pairs(data?.type)+'] performance['+pairs(data?.performance)+'] capabilities['+((data?.capabilities||[]).join(', ')||'待确认')+'] contingency['+(typeof data?.contingency==='string'?data.contingency:'待确认')+'] '+(data?.confirmation_status||(data?.confirmed?'confirmed':'pending_confirmation'));
...
export function render({flow}){
  ...
  const devices=(flow.device_catalog?.items||[]).map(item=>item.subsystem+':'+(item.type?.technology||'unknown')).join(' · ')||'未加载';
