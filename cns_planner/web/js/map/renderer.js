...
export function drawGridTheme(options){
  ...
  for(const item of cache.cells){
    ...
    const semanticStatus=kind==='population'&&hasPopulationQuantity?'passed':attributes?.status;
    ...
    const partial=kind==='population'&&hasData&&attributes.coverage_status==='partial';ctx.globalAlpha=hasData?(partial ? .55 : .72):.48;ctx.fillStyle=hasData?gridTheme.colorForValue(value,breaks,palette):gridTheme.NO_DATA_COLOR;
