...
- 真实数据 readiness：**NOT_READY**；数据块 readiness=`None`。

- NOT READY 原因：`no_project_path_argument`

| 条目 | 状态 | 原因 | 需要的人工动作 |
|---|---|---|---|
| **DATA-1** | `NOT_READY` | `project_evidence_unavailable` | 由数据方给出权威 CRS 证据并写入 reference source_crs（禁止猜测 WGS84/CGCS2000）。 |
| **DATA-2** | `NOT_READY` | `project_evidence_unavailable` | 人工把 .et 转换为 XLSX/CSV 后重新导入；系统不实现 ET parser。 |
| **DATA-3** | `retired` | `not_applicable_by_architecture_decision` | 无人工动作；已按架构决策退役，仅保留 display-only 兼容信息。 |

- reference landing sites：`None` 条 · source CRS 已确认 `None`
- reference routes：`None` 条 · format `None` · status `None`
- AirspacePolicy：`None` 条 · confirmed `None` · V2 readiness `unknown` (`project_evidence_unavailable`)
- 米制度量可用：`False`（未提供可读取的项目：真实数据证据 NOT READY，未造数据。）
...
- λ=0.0: status=passed, length=9428.567 m, risk_exposure=8477.803200490252, turns=2, runtime=14.105 ms
- λ=0.5: status=passed, length=10036.353 m, risk_exposure=2283.9016745366816, turns=6, runtime=55.093 ms
- λ=1.0: status=passed, length=10036.353 m, risk_exposure=2283.9016745366816, turns=6, runtime=77.909 ms
- λ=2.0: status=passed, length=10469.621 m, risk_exposure=2044.6158246426505, turns=6, runtime=104.257 ms
- λ=4.0: status=passed, length=10902.890 m, risk_exposure=1935.829004661576, turns=4, runtime=126.808 ms
- λ=8.0: status=passed, length=10902.890 m, risk_exposure=1935.829004661576, turns=4, runtime=171.535 ms
...
整体：**NOT_READY** · 数据块 readiness `None`
- **DATA-1** `NOT_READY`：project_evidence_unavailable → 由数据方给出权威 CRS 证据并写入 reference source_crs（禁止猜测 WGS84/CGCS2000）。
- **DATA-2** `NOT_READY`：project_evidence_unavailable → 人工把 .et 转换为 XLSX/CSV 后重新导入；系统不实现 ET parser。
- **DATA-3** `retired`：not_applicable_by_architecture_decision → 无人工动作；已按架构决策退役，仅保留 display-only 兼容信息。
