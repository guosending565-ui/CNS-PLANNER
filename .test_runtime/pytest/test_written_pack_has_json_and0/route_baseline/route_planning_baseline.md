...
- 生成时间（UTC）：2026-09-19T07:28:13+00:00
...
- 重复协议：runs=5 · warmup=1 · 统计=min, median, p95, max · seed=None
...
|---|---|---|---|---|
| `open_space` | `route_planner_v1` | `passed` | `True` | 0.343 |
|  | `risk_aware_route_planner_v2` | `passed` | `True` | 18.069 |
...
- 重复统计（runs=5，warmup=1）：min=0.239 · median=0.343 · p95=0.411 · max=0.411 ms（stdev=0.062，仅报告）
...
- 重复统计（runs=5，warmup=1）：min=14.321 · median=18.069 · p95=21.988 · max=21.988 ms（stdev=3.057，仅报告）
- 确定性：result/path fingerprint 稳定性 `True`（distinct result=1 · distinct path=1）· seed `None`
...
```powershell
# 合成 benchmark（重复统计）
python tools/route_planning_baseline.py --runs 5 --warmup 1

# 追加项目级证据（experiments / reference comparison / data readiness）
python tools/route_planning_baseline.py --project <project.json>
```

生成文件：`.test_runtime/pytest/test_written_pack_has_json_and0/route_baseline/route_planning_baseline.json` 与 `.test_runtime/pytest/test_written_pack_has_json_and0/route_baseline/route_planning_baseline.md`。
