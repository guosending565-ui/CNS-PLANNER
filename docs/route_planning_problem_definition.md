...
战略规划不等于 DAA 战术避碰。DAA 处理运行中的 encounter、CPA、告警与响应；不得用战略 route 的可行性替代战术冲突处置。

> Real-Data Enablement V1：QGIS 空域图层与底图均为 `display_only_reference_layer`。它们不参与 V1/V2/V3 风险、规划、readiness、fingerprint、staleness 或 CNS 评估。显式配置的其他 hard constraints 仍然有效。
...
| Constraint | 候选 route 必须满足的硬条件 | 端点合法、显式 hard-constraint BBOX 不相交、风险与三维净空约束 | 风险权重或显示用空域图层 |
| Policy | 把事实解释为规划资格/阈值的显式确认规则 | 风险阈值、V3 高度/净空/机动参数 | 由颜色、图层名或经验猜测出的适飞性 |
| Objective | 在可行集合内比较候选路径的量 | V1 路径代价；V2 距离 + `risk_weight_lambda × risk exposure` | 法规安全结论或硬约束 |

## 3. 输入

- OD：已确认的起点、终点与 canonical `[lon, lat]`。
- airspace：只读显示参考；不产生 allowed/restricted 规划资格。
...
- project context：workspace、MH/T grid、grid risk 及显式 hard constraints 进入证据指纹；airspace eligibility 不进入。
...
2. V2 保留距离、邻接、corner guard、显式 hard constraints 与风险成本；不设置空域 cell/edge gate。
3. 搜索空间 = 工作区有效标准网格 + 真正 explicit hard constraints（不因空域图层或 AirspacePolicy 收缩或扩张）。
4. hard constraints 优先阻断；risk 只能在 hard-constraint 可行集合内优化。
5. 空域数据缺失或变化不阻断规划，也不触发规划结果失效。
6. reference/scenario routes 可以作为事实或候选存在于 hard constraints 之外；只有 operational route 受运行硬约束。

## 6. 当前机制与优化量（冻结事实）

- V1：56×56 经纬度规则格（manifest 默认值）、8 邻域 A*、BBOX hard constraints、共线点简化；不读取 risk、height 或 kinematics。
- V2：MH/T cell-centroid 8 邻域 A*；搜索空间由工作区有效标准网格与显式 hard constraints 决定（**不再**由预计算 allowed graph 限制）；优化量为已有距离与相对风险加权和，`risk_weight_lambda` 语义保持不变；输出完整 grid path，无 smoothing。
...
- **DATA-3 已退役**：AirspacePolicy 不再是 readiness 项；空域图层只保留原始显示。
